...
namespace JurTranspiler.compilerSource.Analysis {
	...
	public partial class Binder {
		...
		public Type BindStructDefinition(StructDefinitionSyntax syntax, HashSet<Error> errors) {
			return symbols.AlreadyBound(syntax)
				       ? symbols.GetBindingFor(syntax)
				       : symbols.MakeBindingFor(syntax, BindStructDefinitionCore(syntax, errors));
		}


		private Type BindStructDefinitionCore(StructDefinitionSyntax syntax, HashSet<Error> errors) {
			...
			var lazyTypeArguments = syntax.TypeParameters.Select(p => new Lazy<Type>(() => BindType(p, errors)));

			var lazyInlinedTypes = hasCycles ? new Lazy<Type>[0] : validInlines.Select(inline => new Lazy<Type>(() => BindType(inline, errors)));
			var lazyFields = syntax.Fields.Select(f => new Lazy<Field>(() => new Field(f, syntax, BindType(f.Type, errors))));
		...
		public Type BindType(ITypeSyntax syntax, HashSet<Error> errors) {
			return symbols.AlreadyBound(syntax)
				       ? symbols.GetBindingFor(syntax)
				       : symbols.MakeBindingFor(syntax, BindTypeCore(syntax, errors));
		}


		private Type BindTypeCore(ITypeSyntax syntax, HashSet<Error> errors) {
			if (syntax is VoidTypeSyntax) return new VoidType();
			if (syntax is ArrayTypeSyntax arrayType) return new ArrayType(BindType(arrayType.ElementType, errors));
			if (syntax is FunctionPointerTypeSyntax functionPointerType) {
				var returnType = BindType(functionPointerType.ReturnType, errors);
				var parameters = functionPointerType.Parameters.Select(param => BindType(param, errors));
			...
			if (syntax is StructTypeSyntax structTypeSyntax) {
				...
				//prepare a typeMap
				var typeArgumentsTypes = structTypeSyntax.TypeArguments.Select(arg => BindType(arg, errors)).ToList();
				var typeParametersTypes = openType.OriginalDefinitionSyntax
				                                  .TypeParameters
				                                  .Select(par => BindType(par, errors))
					...
					if (isDeclared) {
						...
						if (parentDeclaration is FunctionDefinitionSyntax function) {
							...
							return new TypeParameterType(name: typeParameter.FullName,
							                             ...
							                             constraints: validConstraints.Select(x => new Lazy<TypeParameterType>(() => (TypeParameterType) BindType(x.constrain, errors)))
							                                                          .ToImmutableList());
						}
					}
				}
