...
namespace JurTranspiler.compilerSource.Analysis {
	...
	public partial class Binder {
		...
		private void BindFunctionBody(ISyntaxNode scope) {
			...
			if (scope is FunctionDefinitionSyntax notMain)
				CheckForReturnTypeMismatch(notMain);
		}
		...
		private void CheckForDuplicateVariables(ISyntaxNode scope) {
			...
			var duplicatesGroup = localsAndParameters.Select(x => symbols.GetVisibleVariablesInScope(x)
